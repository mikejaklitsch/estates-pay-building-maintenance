...
def generate_io_localization(all_pm_goods, legacy_pm_names=None):
    """
    Generate epbm_ios_l_english.yml:
    Localization entries for each IO to suppress auto-generated placeholder warnings.
    Includes legacy government building PM IOs if present.
    """
    lines = ["\ufeffl_english:"]
    ...
    if legacy_pm_names:
        for pm_name in legacy_pm_names:
            io = f"epbm_pm_{pm_name}"
            lines.append(f' {io}: ""')
            lines.append(f' {io}_desc: ""')
            lines.append(f' diplomatic_status_{io}_name: ""')
...
def find_legacy_pm_names(buildings, pms, current_pm_goods):
    """
    Find PM names exclusively used by government buildings (not shared with current qualifying PMs).
    These need legacy IO type definitions so old save instances remain valid and destroyable.
    """
    legacy_pms = set()

    for bname in GOVERNMENT_BUILDINGS:
    ...
    # Exclude PMs that are still in use by current qualifying buildings
    return sorted(legacy_pms - set(current_pm_goods.keys()))


def generate_legacy_ios(legacy_pm_names):
    """Generate IO type definitions for legacy government building PMs.
    Keeps old save instances valid so they can be cleanly destroyed during migration."""
    lines = [
        "# Auto-generated by tools/generate_building_hooks.py",
        "# Legacy IO types from pre-1.2 government building PMs.",
        "# Kept so old save instances remain valid and destroyable during version migration.",
        "",
    ]
...
def generate_legacy_biases(legacy_pm_names):
    """Generate opinion biases for legacy IOs to suppress engine warnings."""
    lines = [
        "# Auto-generated by tools/generate_building_hooks.py",
        "# Legacy opinion biases for pre-1.2 government building PM IOs.",
        "",
    ]
...
def generate_legacy_destroy(legacy_pm_names):
    """Generate scripted effect to destroy legacy government building PM IOs."""
    lines = [
        "# Auto-generated by tools/generate_building_hooks.py",
        "# Destroys legacy government building PM IOs from pre-1.2 saves.",
        "# Called from epbm_cleanup_legacy_vars during version-triggered rebuild.",
        "",
        "epbm_destroy_legacy_ios = {",
        "\trandom_country = {",
    ...
    for pm_name in legacy_pm_names:
        io_ref = f"international_organization:epbm_pm_{pm_name}"
        lines.append(f"\t\tif = {{")
        lines.append(f"\t\t\tlimit = {{ exists = {io_ref} }}")
        lines.append(f"\t\t\tdestroy_international_organization = {{ target = {io_ref} }}")
        lines.append(f"\t\t}}")
...
def main():
    ...
    # Find legacy PMs exclusive to government buildings (for migration cleanup)
    legacy_pm_names = find_legacy_pm_names(buildings, pms, all_pm_goods)
    if legacy_pm_names:
        print(f"  Legacy government PM profiles: {len(legacy_pm_names)}")
    ...
    # Legacy migration files for government building PM IOs
    if legacy_pm_names:
        legacy_ios = generate_legacy_ios(legacy_pm_names)
        out_path = OUT_IOS / "epbm_legacy_ios.txt"
        out_path.write_text(legacy_ios, encoding="utf-8-sig")
        print(f"  Wrote {out_path.name}")

        legacy_biases = generate_legacy_biases(legacy_pm_names)
        out_path = OUT_BIASES / "epbm_legacy_biases.txt"
        out_path.write_text(legacy_biases, encoding="utf-8-sig")
        print(f"  Wrote {out_path.name}")

        legacy_destroy = generate_legacy_destroy(legacy_pm_names)
        out_path = OUT_EFFECTS / "epbm_legacy_cleanup.txt"
        out_path.write_text(legacy_destroy, encoding="utf-8-sig")
        print(f"  Wrote {out_path.name} ({len(legacy_pm_names)} IOs)")

    loc = generate_io_localization(all_pm_goods, legacy_pm_names)
